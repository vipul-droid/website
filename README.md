...
Any contributions are appreciated, no matter how big or small ,it is. The Cake site consists of several different sections and each one is described below.
...
> PLEASE NOTE: Addin authors does not have to manually create YAML files as there is an automated process that scans nuget.org twice per day to find all addins that follow the recommended naming convention (which is `Cake.xxx` where xxx describes the functionality provided by the addin) and generates the appropriate YAML content based on the metadata for the NuGet package.
